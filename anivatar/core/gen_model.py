...
class Generator(nn.Module):
    def __init__(
        ...
        self.label_embed = EqualLinear(lb_size, style_dim)
    ...
    def mean_latent(self, n_latent, labels):
        latent_in = torch.randn(
            n_latent, self.style_dim, device=self.input.input.device
        )
        labels = self.label_embed(labels)
        latent_in = torch.cat([latent_in, labels], dim=1)
        latent = self.style(latent_in).mean(0, keepdim=True)
        return latent

    def get_latent(self, styles, labels):
...
class Discriminator(nn.Module):
    def __init__(
        ...
        self.pixel_norm = PixelNorm()
        self.label_embed = EqualLinear(lb_size, channels[4])

        convs = [ConvLayer(3, channels[img_size], 1)]
